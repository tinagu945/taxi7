...
#         plot_res(final, title)
        ...
        if total >= best:
            best=total
            torch.save(model.model.state_dict(), 'logs/cartpole_best.pt')
        print('best at epoch ', np.argmax(final))
        if episode % 10==0:
            torch.save(model.model.state_dict(), 'logs/cartpole_'+str(episode)+'_'+str(total)+'.pt')
