    ...
    for sasr in SASR:
        ...
        for state, action, next_state, reward in sasr:
            ...
            den_discrete.feed_data(
                state, next_state, initial_state, policy_ratio, discounted_t)
...
def off_policy_evaluation_density_ratio(SASR, policy0, policy1, density_ratio, gamma):
    ...
    for sasr in SASR:
        ...
        for state, action, next_state, reward in sasr:
            policy_ratio = policy1[state, action]/policy0[state, action]
            total_reward += density_ratio[state] * \
                policy_ratio * reward  # * discounted_t
            # density_ratio[state] * policy_ratio ######* discounted_t
            self_normalizer += 1.0
            # discounted_t = gamma
    return total_reward / self_normalizer


def double_evaluation_density_ratio(SASR, policy0, policy1, density_ratio, gamma, q_table, e):
    total_reward = 0.0
    self_normalizer = 0.0
    for sasr in SASR:
        discounted_t = gamma
        for state, action, next_state, reward in sasr:
            policy_ratio = policy1[state, action]/policy0[state, action]
            total_reward += density_ratio[state] * policy_ratio * (reward+gamma*np.sum(
                pi[next_state]*q_table[next_state, :]) - q_table[state, action])  # * discounted_t
            # density_ratio[state] * policy_ratio ######* discounted_t
            self_normalizer += 1.0
            # discounted_t = gamma
    return total_reward / self_normalizer
...
def importance_sampling_estimator(SASR, policy0, policy1, gamma):
    ...
    for sasr in SASR:
        ...
        self_normalizer = 0.0
        for state, action, next_state, reward in sasr:
            log_trajectory_ratio += np.log(policy1[state, action]) - \
                np.log(policy0[state, action])
            total_reward += reward * discounted_t
            self_normalizer += discounted_t
            discounted_t *= gamma
...
def importance_sampling_estimator_stepwise(SASR, policy0, policy1, gamma):
    ...
    for sasr in SASR:
        ...
        self_normalizer = 0.0
        for state, action, next_state, reward in sasr:
            step_log_pr += np.log(policy1[state, action]) - \
                np.log(policy0[state, action])
    ...
    for sasr in SASR:
        ...
        for state, action, next_state, reward in sasr:
            ...
            est_reward += np.exp(step_log_pr) * \
                (reward - q_table[state, action])*discounted_t
            est_reward += np.exp(step_log_pr_previous) * \
                (np.sum(pi[state]*q_table[state, :]))*discounted_t
            self_normalizer += discounted_t
            discounted_t *= gamma
        est_reward /= self_normalizer
        mean_est_reward += est_reward
...
def weighted_importance_sampling_estimator_stepwise(SASR, policy0, policy1, gamma):
    ...
    for sasr in SASR:
        ...
        self_normalizer = 0.0
        for state, action, next_state, reward in sasr:
            log_pr = np.log(policy1[state, action]) - \
                np.log(policy0[state, action])
            if log_policy_ratio:
    ...
    for i in range(REW.shape[0]):
        est_reward += np.sum(rho[i]/np.mean(rho, axis=0)
                             * REW[i])/self_normalizer
...
#         SAS, f, avr_reward = roll_out(
#             n_state, env, pi, num_trajectory, truncate_size)
    ...
    for state in range(n_state):
        for next_state in range(n_state):
            for action in range(n_action):
                Tpi[state, next_state] += T[state, action,
                    next_state] * pi[state, action]
    dt = d0/np.sum(d0)
...
def model_based(n_state, n_action, SASR, pi, gamma):
    ...
    for i in range(2000):
        ...
        for state in range(n_state):
            for action in range(n_action):
                Q_table_new[state, action] = R[state, action] + \
                    gamma*np.sum(T[state, action, :]*V_table)
...
def run_experiment(n_state, n_action, SASR, pi0, pi1, gamma):
    ...
    # weighted_importance_sampling_estimator(SASR, pi0, pi1, gamma)
    est_WIST = 0.0
    # weighted_importance_sampling_estimator_stepwise(SASR, pi0, pi1, gamma)
    est_WISS = 0.0

    # Q-table is estimated from model_based
    est_model_based, Q_table = model_based(n_state, n_action, SASR, pi1, gamma)
    ddd = np.load("emp_hist.npy").reshape(-1)
    est_model_double = np.sum(np.sum(Q_table*pi1, 1).reshape(-1)*ddd)*(
        1-gamma)+double_evaluation_density_ratio(SASR, pi0, pi1, w, gamma, Q_table, pi1)
    est_drl = double_importance_sampling_estimator(
        SASR, pi0, pi1, gamma, Q_table, pi1)
    # Q-table misspecified
    Q_table_mis = Q_table + \
        np.random.normal(1.0, 1.0, n_state *
                         n_action).reshape([n_state, n_action])
    est_model_based_mis_q = np.sum(
        np.sum(Q_table_mis*pi1, 1).reshape(-1)*ddd)*(1-gamma)
    est_model_double_mis_q = np.sum(np.sum(Q_table_mis*pi1, 1).reshape(-1)*ddd)*(
        1-gamma)+double_evaluation_density_ratio(SASR, pi0, pi1, w, gamma, Q_table_mis, pi1)
    est_drl_mis_q = double_importance_sampling_estimator(
        SASR, pi0, pi1, gamma, Q_table_mis, pi1)
    # ratio misspecified
    w_mis = w + np.random.normal(1.0, 1.0, n_state)
    est_DENR_mis_w = off_policy_evaluation_density_ratio(
        SASR, pi0, pi1, w_mis, gamma)
    est_model_double_mis_w = np.sum(np.sum(Q_table*pi1, 1).reshape(-1)*ddd)*(
        1-gamma)+double_evaluation_density_ratio(SASR, pi0, pi1, w_mis, gamma, Q_table, pi1)
...
if __name__ == '__main__':
    estimator_name = ['On Policy', 'Density Ratio', 'Naive Average', 'IST', 'ISS', "DRL", 'WIST', 'WISS',
                      'Model Based', 'double', "Model Base misq", "Double misq", "mis_q", "DR wmis", "Double misq"]
    length = 5
    env = taxi(length)
    n_state = env.n_state
    ...
    # False, default=num_trajectory)
    parser.add_argument('--nt', type=int, required=True)
    # default=truncate_size)
    parser.add_argument('--ts', type=int, required=True)
    parser.add_argument('--gm', type=float, required=True)  # default=gamma)
    ...
    for k in range(1):
        ...
        est_IST = importance_sampling_estimator(
            SASR0, pi_behavior, pi_target, gm)
        est_ISS = importance_sampling_estimator_stepwise(
            SASR0, pi_behavior, pi_target, gm)
        print('est_IST', est_IST, 'est_ISS', est_ISS)

        res[1:, k] = run_experiment(n_state, n_action, np.array(
            SASR0), pi_behavior, pi_target, gm)
        ...
        # for i in range(15):
        #     print('  ESTIMATOR: ' +
        #           estimator_name[i] + ', rewards = {}'.format(res[i, k]))
        #     print('----------------------')
        #     sys.stdout.flush()
