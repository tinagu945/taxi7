...
def run_experiment(n_state, n_action, SASR, pi0, pi1, gamma):
    ...
    x, w = train_density_ratio_q(SASR, pi0, pi1, den_discrete, gamma,pi1)
    x = x.reshape(-1)
    w = w.reshape(-1)
    
    est_model_based, Q_table = model_based(n_state, n_action, SASR, pi1, gamma)

    ####est_DENR2 = off_policy_evaluation_density_ratio_q(SASR, pi0, pi1, w, gamma)
    

    return est_DENR #, est_DENR2
...
res = np.zeros((3, 20), dtype = np.float32)
for k in range(10):
    print k
    ...
    for i in range(len(estimator_name)):
        ...
        print('----------------------')
        sys.stdout.flush()
    np.save('result/un_ver={}nt={}ts={}gm={}.npy'.format(ver,nt,ts,gm), res)
